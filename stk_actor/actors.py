...
def item_value(item_type):
    ...
    if item_type in [1, 3, 4, 6]:  # bonus, nitro, easter
        return 1.0   # BON
    elif item_type in [2, 5]:  # banana, bubble_gum
        return -1.0  # MAUVAIS
    else:  # 0 = none
        return 0.0
...
def nearest_items_features(items_position, items_type, max_dist=50.0, n_items=3):
    """
...
class MinimalEssentialObsWrapper(gym.Wrapper):
    ...
    def _process_obs(self, obs):
        ...
        itemsss = nearest_items_features(obs['items_position'], obs['items_type'], max_dist=20.0, n_items=3)
        ...
        off_track = 1.0 if abs(float(obs['center_path_distance'][0])) > (curr_w / 2) + 0.5 else 0.0
